---
layout: doc
---

# Todos

Dieses Dokument enthält eine Liste von Aufgaben, die noch erledigt werden müssen.

## 10.metrics\code.md

- [MC3 Qualitiative Metriken](10.metrics\code.md#qualitiative-metriken)

## 10.metrics\index.md

- [https://vitepress.dev/reference/default-theme-home-page](10.metrics\index.md)

## 4.designpatterns\behavioral.md

- [Entwurfsmuster - Verhaltensmuster](4.designpatterns\behavioral.md)

## 4.designpatterns\creational.md

- [DPC1 Singleton](4.designpatterns\creational.md#singleton)
- [DPC5 Prototype](4.designpatterns\creational.md#prototype)

## 4.designpatterns\structural.md

- [DPS3 Composite](4.designpatterns\structural.md#composite)
- [DPS5 Facade](4.designpatterns\structural.md#facade)
- [DPS6 Flyweight](4.designpatterns\structural.md#flyweight)

## 5.refactoring\all.md

- [RFA6 Variablen extrahieren](5.refactoring\all.md#variablen-extrahieren)
- [RFA14 Doppelter Code](5.refactoring\all.md#doppelter-code)

## 5.refactoring\java.md

- [Refactoring von Java](5.refactoring\java.md)

## 5.refactoring\javascript.md

- [Refactoring von JavaScript und TypeScript](5.refactoring\javascript.md)

## 5.refactoring\vue.md

- [Refactoring von Vue 3](5.refactoring\vue.md)

## 6.languages\vue\index.md

- [Richtlinien für Vue](6.languages\vue\index.md)

## 8.accessibility\index.md

- [https://vitepress.dev/reference/default-theme-home-page](8.accessibility\index.md)

## 8.accessibility\ui.md

- [Einleitung](8.accessibility\ui.md#einleitung)


